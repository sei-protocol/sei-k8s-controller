...
// nodeKeyVolumes mounts only on the production StatefulSet pod. The bootstrap
// Job uses an ephemeral node ID generated by `seid init` so its lifecycle
// drama (snapshot-restore failures, halt-on-130) doesn't pollute the
// validator's permanent libp2p reputation.
