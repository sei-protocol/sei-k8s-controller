...
// package's configPatchKeyAllowed, sdk/sei.configPatchKeyAllowed, and the CRD's CEL
// family rule on SeiNodeTaskSpec — because the SDK's apimachinery-free boundary
...
// sibling table in sdk/sei (configpatch_allowlist_test.go). The CEL leg is covered by
// the envtest admission cases.
...
// TestConfigPatchKeyAllowedParity pins internal/task.configPatchKeyAllowed to the
// shared allowlist contract. sdk/sei has a mirror test against the identical table;
// a divergence between the two Go copies surfaces as one of the two tests failing.
