...
// Package envtest_test runs SeiNode schema-validation checks against a real
// kube-apiserver started by controller-runtime's envtest harness. Build-tagged
// so default `go test ./...` skips it — the harness depends on a downloaded
// apiserver/etcd binary set (see Makefile `setup-envtest`).
//
// OpenAPI structural validation (minItems, listType=set uniqueness, item
// patterns) is enforced by the apiserver, not the fake client, so it can only
// be exercised here.
