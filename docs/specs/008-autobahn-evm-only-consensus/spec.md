...
- **SC-003**: The rendered `config.toml` and `app.toml` of an EVM-only validator match Requirement 3 item 2, its pod carries no `cosmos-exporter` container, a `spec.configOverrides` entry naming an engine key is rejected at admission, and a `spec.configValues` entry naming one is applied and produces a Warning event.
