...
2. WHEN the producing signal exceeds the observed height AND the recorded observation time is older than a fraction of the grace window, THE network controller SHALL record the new height and the observation time as the observed height on the SeiNetwork status.
...
4. WHILE the children are `Running`, AND the network controller has recorded no observed height yet, or the observed height has not advanced since it was first recorded, THE network controller SHALL report the network phase as `Initializing`.
...
5. WHILE the network has at least one child and every child read time is stale, THE network controller SHALL set the `Producing` reason to signal unreadable, and let the grace window drive the phase.
...
- The committed height on the SeiNode status carries the time the node controller read it. The network controller excludes a child whose read time is older than a staleness bound, so a stale minority does not read as a stall. The staleness bound exceeds the node poll interval, which is about 30 seconds, with margin, so a child is not counted stale in the moments before its next refresh and a healthy chain does not flip the reason to `SignalUnreadable` each cycle. When every child is stale — a node controller that stopped reconciling freezes them all at once — the whole signal is unreadable: the observed height stops advancing, the grace window drives the phase to `Degraded`, and the `Producing` reason is `SignalUnreadable`, so the runbook reads a controller outage rather than a chain stall. Both an unreadable signal and a real stall reach `Degraded`, which keeps the bound; the reason tells them apart.
...
- `Ready` is no longer monotonic: a network can leave `Ready`. In-repo consumers read the current meaning — the `WaitReady` helper and the phase gauge — so leaving `Ready` is now an expected transition, not a fault. A consumer that waits for `Ready` must handle `Degraded`: `WaitReady` treats every non-`Ready`, non-`Failed` phase as keep-polling, so a produced-then-wedged network in `Degraded` would block a caller to its context deadline. `WaitReady` fails fast on a `Degraded` that carries the `HeightStalled` reason rather than poll, and updating that consumer is part of this change.
