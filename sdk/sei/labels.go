...
// truth. The controller's seitask and seictl copies are unexported (internal/),
// so the SDK authors these once here. Changing a value is a fleet-wide breaking
// change: chaos selectors, follower-discovery queries (node list
// -l sei.io/seinetwork=<net>,sei.io/role=node), and seictl all match the exact
// literals.
const (
...
// from seictl ("seictl") and seitask ("seitask-provision-node"). Stable: renaming
// it orphans field ownership on objects the SDK already created.
