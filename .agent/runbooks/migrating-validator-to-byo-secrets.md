...
**Ad-hoc debugging on seid (`seid keys list`, `seid tx sign`) — a further, separate trade-off, not just "more of the same."** By default the keyring is sidecar-only — `seid` never sees it, even though `shareProcessNamespace` already lets a compromised seid reach it via `/proc` as described above. Setting `operatorKeyring.secret.exposeToSeid: true` additionally mounts the keyring directory and its passphrase directly onto the seid main container, so an operator can run `seid keys`/`seid tx sign` from inside that container without going through `/proc`. This drops the bar from "code execution in seid" to "any file or env read in seid" (a much more common bug class) and makes the exposure a standing one that survives seid restarts, rather than requiring the sidecar to still be running. Scope is main-container-only: the controller's containment guard hard-fails the reconcile if this material ever lands on `seid-init` or any other container, regardless of this flag. There is no chain-id or environment gate on this field; it is a per-node call left to the operator setting it.

**If you rotate the passphrase or keyring Secret after enabling `exposeToSeid`, restart the whole pod.** The keyring directory hot-reloads (no `subPath`), but the passphrase env var is fixed at each container's own create time — and `seid` and `sei-sidecar` now independently freeze it, so a rotation can leave them briefly disagreeing about which passphrase is current (whichever container has restarted more recently wins). This isn't a slashing risk (operator-tx signing isn't consensus signing), but it can invert your debugging signal: a freshly-restarted `seid` may report keys as readable while the longer-lived sidecar — the one actually signing on-chain — is the one that's stale and failing. Restarting both containers after a rotation resyncs them.
  ...
  operatorKeyring:
    secret:
      ...
      passphraseSecretRef:
        ...
        key: passphrase
```
Controller ≥ `5d03f33` runs `validate-operator-keyring` on bring-up. To vote **on the node itself** (in the `seid` container, e.g. via `kubectl exec`) rather than through the sidecar, the SeiNode also needs `operatorKeyring.secret.exposeToSeid: true` (see the "Ad-hoc debugging on seid" trade-off above) — without it, `seid` has no keyring mount and the command below fails empty, not with a clear error: `seid tx gov vote <id> <option> --from node_admin --keyring-backend file --keyring-dir $SEI_HOME/keyring-file`.
