...
**Security tradeoff — decide before mounting.** The operator account key controls the validator's *treasury and governance* (withdraw-commission, move self-stake, change withdraw address, vote, edit, unjail) — a strictly larger blast radius than the consensus key, which only risks slashing. The pod runs `shareProcessNamespace: true` and both `seid` and `sei-sidecar` run as the same UID, so a code-exec compromise of the internet-exposed seid container can already read the passphrase out of `/proc/<sidecar>/environ` and the encrypted keyring out of `/proc/<sidecar>/root/...` — plain same-UID file reads, no `CAP_SYS_PTRACE` or ptrace-attach required. **If you only need voting, the safer pattern is an `authz` `MsgVote`-only grant to a dedicated low-privilege key** — mount *that*, keep the full operator key offline; vote inheritance still attributes to the operator account (`MsgExec` sets `voter = granter`). Mount the full key only with eyes open.

**Ad-hoc debugging on seid (`seid keys list`, `seid tx sign`) — a further, separate trade-off, not just "more of the same."** By default the keyring is sidecar-only — `seid` never sees it, even though `shareProcessNamespace` already lets a compromised seid reach it via `/proc` as described above. Setting `operatorKeyring.secret.seidMount: true` additionally mounts the keyring directory and its passphrase directly onto the seid main container, so an operator can run `seid keys`/`seid tx sign` from inside that container without going through `/proc`. This drops the bar from "code execution in seid" to "any file or env read in seid" (a much more common bug class) and makes the exposure a standing one that survives seid restarts, rather than requiring the sidecar to still be running. Scope is main-container-only: the controller's containment guard hard-fails the reconcile if this material ever lands on `seid-init` or any other container, regardless of this flag. There is no chain-id or environment gate on this field; it is a per-node call left to the operator setting it.

**Flagless `seid` on the node — what now works without flags, and the boundary.** The pod's `HOME` is `/home/nonroot` and the data PVC mounts at `$HOME/.sei`, so a bare `seid keys list` / `seid tx sign` (no `--home`) resolves the data dir the same way it does on an EC2 host. The controller also writes `keyring-backend` into `<data>/config/client.toml` at every boot (so `--keyring-backend` is redundant), and because `$HOME/.sei` is now the keyring home the SDK finds the keyring without `--keyring-dir` either. This makes flagless on-node signing/inspection Just Work **for a test-backend validator** (no `operatorKeyring.secret`) — its keyring lives on the data PVC `seid` already mounts. **The boundary:** a `.secret` (file-backend) validator **without** `seidMount` gets `keyring-backend = "file"` in `client.toml`, but the keyring directory and passphrase are mounted on the sidecar only — so a bare `seid keys list` in the `seid` container comes up empty (no key material), not with a clear error. For those nodes either `kubectl exec` into the `sei-sidecar` container, or set `operatorKeyring.secret.seidMount: true` (with that set, the keyring + passphrase are on `seid` too and flagless commands work end-to-end). The flags are redundant only for commands run **inside the `seid` container**. The host-side key import in step 2 below still needs its explicit `--keyring-backend file --keyring-dir "$KR"` flags: it runs against a throwaway keyring dir with no `client.toml` and no HOME convergence.

**If you rotate the passphrase or keyring Secret after enabling `seidMount`, restart the whole pod.** The keyring directory hot-reloads (no `subPath`), but the passphrase env var is fixed at each container's own create time — and `seid` and `sei-sidecar` now independently freeze it, so a rotation can leave them briefly disagreeing about which passphrase is current (whichever container has restarted more recently wins). This isn't a slashing risk (operator-tx signing isn't consensus signing), but it can invert your debugging signal: a freshly-restarted `seid` may report keys as readable while the longer-lived sidecar — the one actually signing on-chain — is the one that's stale and failing. Restarting both containers after a rotation resyncs them.
  ...
  operatorKeyring:
    secret:
      ...
      passphraseSecretRef:
        ...
        key: passphrase
```
Controller ≥ `5d03f33` runs `validate-operator-keyring` on bring-up. To vote **on the node itself** (in the `seid` container, e.g. via `kubectl exec`) rather than through the sidecar, the SeiNode needs `operatorKeyring.secret.seidMount: true` (see the "Ad-hoc debugging on seid" trade-off above). With that set the flagless form resolves end-to-end (`client.toml` carries the backend and `$HOME/.sei` is the keyring home): `seid tx gov vote <id> <option> --from node_admin`. Without `seidMount`, `seid` has no keyring in its container and this comes up empty with no clear error — vote from the `sei-sidecar` container instead.
