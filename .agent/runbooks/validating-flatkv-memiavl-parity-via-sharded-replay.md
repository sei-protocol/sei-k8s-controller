...
- **`evm.max_trace_lookback_blocks: -1`** — L2 touched-key resolution runs `debug_traceBlockByNumber`, which is capped by default (a finite cap surfaces as `beyond max lookback of N`); `-1` = unlimited. (L2 isn't the flatKV verdict — §1 — but keeping it *determinate* still earns its keep: it's the SS-level check that the two nodes agree on the same replayed history, and an all-indeterminate L2 would hide a genuine history/SS divergence.)
