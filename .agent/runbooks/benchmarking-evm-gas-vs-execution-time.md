...
**A narrower, EVM-adjacent metric exists too.** `app_block_process_duration_seconds{type="optimistic_concurrency"}` (the `app_` prefix and `_seconds` suffix are load-bearing — `app/metrics.go`'s `blockProcessDuration` registers as `app_block_process_duration` with `WithUnit("s")`; the bare `block_process_duration` name a PromQL query might guess returns nothing) covers just the parallel transaction-execution phase, excluding begin/end-blockers and commit — a more targeted signal for pure gas-vs-compute work, since gas is charged during tx execution, not the blockers.

**And one already-existing, EVM-only signal worth knowing about: `run_msg_latency`** (`sei-cosmos/baseapp/metrics.go`), a histogram labeled by exact message-type URL (`sdk.MsgTypeURL(msg)`, `baseapp.go:1060-1061`), recorded at `baseapp.go`'s message-routing loop. **The label value is the proto message-type name, not the gRPC service/method path** — confirm this against `MsgTypeURL`'s actual output before filtering, since the two look similar and only one exists: `run_msg_latency_seconds{type="/seiprotocol.seichain.evm.MsgEVMTransaction"}` (`x/evm/types/tx.pb.go`'s registered message name), **not** `/seiprotocol.seichain.evm.Msg/EVMTransaction` (that string is the proto `service Msg { rpc EVMTransaction }` path, `proto/evm/tx.proto`, which `MsgTypeURL` never emits — a query using it silently matches zero series). Otherwise a clean, low-cardinality (the label is message type, not per-tx), EVM-only per-message-execution-time histogram, zero code change required, already excluding ante-handler time (a sibling span/timer, not included in this window).
...
**Built and tested it anyway, with real data, to settle the efficacy question rather than reason about it abstractly.** Stood up two tracing-enabled replayer nodes in `eng-fromtherain` (a wrapper image backgrounding an in-process Jaeger collector before exec'ing the real seid binary, so AppHash fidelity against pacific-1 was unaffected) — one running default concurrent OCC execution, one with the SeiNode CRD's `chain.concurrency_workers=1` override forcing strict serial execution — both restored from the identical S3 snapshot, so both replayed the identical real historical chain. (`chain.concurrency_workers` is the dotted override-path key in `spec.overrides`, per the unified sei-config schema; the sidecar translates it into the actual rendered app.toml key, `concurrency-workers` — hyphenated, no section prefix — which is what `seid` itself reads. Verify against the rendered config on the pod, not just the override you wrote, before trusting that a setting took effect.)

**The wrapper only works by shadowing `seid` on `$PATH` — an image `ENTRYPOINT`/`CMD` change would silently do nothing.** The controller never invokes whatever entrypoint the image declares: `buildSidecarMainContainer` sets the pod's `Command`/`Args` directly to a fixed `exec seid start --home <dataDir>` (`sei-k8s-controller`'s `internal/noderesource/noderesource.go`), bypassing any image-level `ENTRYPOINT` entirely. The wrapper has to replace the actual `seid` binary at the path the shell resolves it from (rename the real binary aside, put the wrapper script in its place) — an entrypoint-based wrapper would simply never run, `seid` would start without `--tracing`, and nothing would error. Exactly the kind of silent-failure trap this runbook exists to flag.
