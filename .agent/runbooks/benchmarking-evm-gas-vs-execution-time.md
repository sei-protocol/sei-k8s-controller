...
2. **Pin to a dedicated node** — `CAP_PERFMON` reads are node-global, not pod-scoped, so the isolation boundary is the node's kernel, not the namespace. The `sei-node`/`sei-archive` taint is **not** single-tenancy: it repels *non-Sei* pods, but other Sei-managed pods and the node's own DaemonSets (`eks-pod-identity-agent`, `cilium-agent`) still co-reside — and the pod-identity agent's cached AWS creds are themselves kernel-readable from a `CAP_PERFMON` pod. The actual single-tenant primitive is the **`sei.io/dedicated-node: "true"` annotation** on the SeiNode (hostname anti-affinity — the pod refuses to share a node with any other Sei-managed pod); set it on your replayer/archive-fork node. Even then two residuals remain and are accepted-risk for a dev one-off, not closed: (i) the node's IAM role reachable via IMDS, unless the node's `EC2NodeClass` pins `httpPutResponseHopLimit: 1` (the default harbor class does not; `clusters/dev/sei-omnigent/nodepool-sandbox.yaml` is the pinned template), and (ii) the co-resident DaemonSets above. If either is unacceptable for your run, that is the signal to escalate to the dedicated profiling node pool (step 4 / the tracked hardening, `PLT-801`), not to proceed.
3. **Scope the capability grant to the minimum**, not `hostPID`: `kubectl debug --target=seid -it <target-node>-0 --image=<bpftrace-image> --profile=sysadmin` — `--target=<container>` joins only that pod's PID namespace. Prefer `CAP_BPF`+`CAP_PERFMON` over the broader `sysadmin` profile where possible — **but the caps are not enough on their own.** The container's `RuntimeDefault` seccomp profile blocks `bpf()` and `perf_event_open()` behind `CAP_SYS_ADMIN`, so a pod holding only `CAP_BPF`+`CAP_PERFMON` under the default seccomp profile gets `EPERM` on the very syscalls bpftrace/BCC need. `--profile=sysadmin` works precisely because it *also* sets `seccompProfile: Unconfined`; the moment you drop to the two caps you must set `seccompProfile: Unconfined` (or a curated Localhost profile allowlisting at least `bpf`, `perf_event_open`, and `ioctl`) explicitly, or the narrowed profile silently fails and reads like a missing-capability bug. The split caps themselves are sufficient once seccomp is handled — kernel ≥5.8 (Bottlerocket's is) gates `bpf()`/`perf_event_open()` on `CAP_BPF`/`CAP_PERFMON`, and ≥5.11 uses memcg BPF accounting so no `RLIMIT_MEMLOCK` bump is needed — no `CAP_SYS_ADMIN` required.
