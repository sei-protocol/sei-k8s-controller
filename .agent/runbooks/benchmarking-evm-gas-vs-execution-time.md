...
2. **Pin to a dedicated node — but know what "dedicated" does and doesn't buy.** `CAP_PERFMON` reads are node-global, not pod-scoped, so the isolation boundary is the node's kernel, not the namespace. The `sei-node`/`sei-archive` taint is **not** single-tenancy: it repels *non-Sei* pods, but the node's own DaemonSets (`eks-pod-identity-agent`, `cilium-agent`) tolerate it and co-reside, and the pod-identity agent's cached node-role creds are kernel-readable from a `CAP_PERFMON` pod. The single-tenant primitive *with respect to other Sei pods* is the **`sei.io/dedicated-node: "true"` annotation** on the SeiNode (bidirectional hostname anti-affinity); set it on your replayer/archive-fork node. **It does not evict the DaemonSets and it does not close the residuals below — which these caps make real and which are not fixable from your pod spec:**
   - **Treat the node IAM role as reachable** from any pod holding `CAP_BPF`+`CAP_PERFMON`: the pod-identity agent's creds are readable from kernel memory *independent of IMDS*, and in a PSS-less `eng-<alias>` namespace a pod may also set `hostNetwork: true` and reach IMDS at hop 0. An `EC2NodeClass` IMDS `httpPutResponseHopLimit: 1` (e.g. `clusters/dev/sei-omnigent/nodepool-sandbox.yaml`, a *dev-cluster* class — not wired to `sei-node`/`sei-archive`) closes only the *unprivileged pod-netns* IMDS path; per `clusters/prod/walle/walle-networkpolicy.yaml` the real control is hop-limit **plus** a restricted PSS that `eng-<alias>` lacks, and neither touches the kernel-memory path. So the node role is reachable regardless of the hop limit.
   - The co-resident DaemonSets above, and any other privileged co-tenant that lands on the node.

   **These residuals are not yours to self-accept — surface them at the step-1 sign-off.** If they are unacceptable for your run, that is the signal to escalate to the dedicated profiling namespace (step 4 / the tracked hardening, `PLT-801`) — **not to proceed.**
3. **Scope the capability grant to the minimum**, not `hostPID`: `kubectl debug --target=seid -it <target-node>-0 --image=<bpftrace-image> --profile=sysadmin` — `--target=<container>` joins only that pod's PID namespace. Prefer `CAP_BPF`+`CAP_PERFMON` over the broader `sysadmin` profile where possible, but the narrowed grant needs two things `sysadmin` gave you implicitly:
   - **A uprobe on `seid` also needs `CAP_SYS_PTRACE`.** Kernel-side probes (`offcputime`, `profile:hz`) run on `CAP_BPF`+`CAP_PERFMON` alone. But a `uprobe`/`uretprobe` on `seid` (§7, the headline probe) attaches through `/proc/<seid-pid>/root/usr/bin/seid`, which is `ptrace_may_access`-gated — and `seid` runs **non-root**, so a uid-0 debug container needs `CAP_SYS_PTRACE` too (or must run as `seid`'s uid). Without it the uprobe fails at path/symbol resolution with `EACCES`/`EPERM` — the same "missing-capability bug" trap, for a cap you didn't grant.
   - **`bpf()`/`perf_event_open()` must not be seccomp-blocked.** A `RuntimeDefault` seccomp profile gates both behind `CAP_SYS_ADMIN`, so *if* RuntimeDefault is in force a two-cap pod gets `EPERM`; lift it by setting **`seccompProfile: Unconfined`** (or a curated Localhost profile adding `bpf` and `perf_event_open` — `ioctl` is already allowed). Two caveats: on harbor today the ephemeral debug container most likely runs **Unconfined already** (no PSS forces RuntimeDefault; Bottlerocket's `seccomp-default` is off), so the predicted `EPERM` may never appear — don't misdiagnose its absence as a cap bug. And `--profile=sysadmin` reaches seccomp-unconfined as a *side-effect* of `privileged: true` (not an explicit `seccompProfile` field), so on the **seccomp axis the split-caps path is no stricter than `sysadmin`** — both end unconfined on a shared node; the isolation you buy by narrowing is in the capability set, not seccomp.

   The split caps are otherwise correct for the kernel: ≥5.8 gates `bpf()`/`perf_event_open()` on `CAP_BPF`/`CAP_PERFMON` (no `CAP_SYS_ADMIN`), and ≥5.11 memcg BPF accounting drops the `RLIMIT_MEMLOCK` bump — though BPF-map memory now counts against the pod's cgroup limit, so a large map can OOM the container.
